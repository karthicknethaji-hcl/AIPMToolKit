...
function buildRequirementAgentTurnPrompt(sessionContext,liveDraftMd,chatHistory,userMessage,docContext,uploadedDocText,uploadedDocName,streamingMode,retrievedDocContext){
  ...
  const usr='DISCOVERY MAP (every value chain stage, with its metrics/process areas — the only valid source of an EXISTING metric/process area name for the "will be created — under:" tag):\n'+dmStr+'\n\n'
    ...
    + 'COVERAGE SCAN - run this every turn, after everything above: look at the CURRENT BRIEF and list which of the 11 sections still show "_Yet to be discussed_" (no real content yet). For each, first check the FULL CONVERSATION SO FAR (not just this turn\'s message) for real information that already satisfies that section\'s real-basis rule - if the PM said something relevant in an earlier turn that never got written in, include that section in sectionUpdates now to backfill it. This retroactive check runs every turn, not only when something new is said. After backfilling whatever you can, if unaddressed sections still remain AND openQuestions is empty this turn AND no PM opt-out is in effect, close your chatReply by naming ONE specific still-open section and inviting the PM to cover it next (e.g. "Want to walk through User Journeys next, or is there a Non-Functional Requirement worth capturing?"). This is plain conversational text in chatReply, NOT the clarifyingQuestions field - it does not count against clarifyingQuestions\' frequency cap and does not need options. Skip this nudge on a turn where: you already used a clarifyingQuestions entry, the PM is still clearly mid-thought on something else, or every section already has real content.\n\n'
    + _raClarifyingQuestionsRules()+'\n\n'
    + (streamingMode
