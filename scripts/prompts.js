...
// 11 numbered sections (matching the PM's own reference product-spec
// convention, "## N. <Name>") — this brief is meant to be the single
// reference document driving every later stage of the build (Capability
// Canvas, Feature Canvas, Story Canvas, etc.), not just a scoping summary.
// Target Users, User Journeys, and Non-Functional Requirements are net-new;
// Problem Statement is also net-new (previously folded into Summary).
// Risks/dependencies (previously paired with Out of Scope) now live inside
// Assumptions, per explicit product decision — not a 12th section.
function _raSectionContentRules(){
  return 'LIVE DRAFT SECTION RULES (apply to every section, every turn - liveDraftMd must always contain ALL of these sections, in this order, even if a section is thin early in the conversation. Headings are NUMBERED exactly as shown - "## 1. Requirement Summary", "## 2. Problem Statement", etc. - this brief is meant to serve as the single reference document driving every later stage of the product build, so completeness and specificity matter more here than anywhere else in this app):\n'
    + '- "## 1. Requirement Summary": one tight paragraph - what release/scope this conversation covers, in plain product language.\n'
    + '- "## 2. Problem Statement": 2-4 sentences naming the SPECIFIC user or business problem this release addresses - the actual friction/gap, why it matters now, and who is affected. Never generic ("improve the experience") - name the concrete pain point. Mark the paragraph "(inferred - confirm with PM)" when the PM has not stated the problem explicitly and this is your own inference from Discovery Map/product context.\n'
    + '- "## 3. Success Criteria": what success looks like for this release - bullet list of concrete, measurable outcomes or signals (not generic platitudes like "improve user experience"). If the user has not stated success criteria yet, infer reasonable candidates from the Discovery Map context and mark each inferred bullet "(inferred - confirm with PM)".\n'
    + '- "## 4. Capabilities": one sub-heading per touched capability, with a short bullet list under each sub-heading of what changes for that capability in this release. Tag each sub-heading in ONE of these two EXACT forms:\n'
    ...
    + '- "## 5. Features": a per-capability list - one "### <Capability Name>" sub-heading per touched capability, exactly matching the capability names used under "## 4. Capabilities" above - followed by one bullet per feature in this EXACT format: "- <Feature Name> (new feature): <requirement narrative>" or "- <Feature Name> (existing feature): <requirement narrative>" (use "(existing feature)" only for a feature that already exists on that capability today and is being referenced, not newly proposed; use "(new feature)" for everything genuinely new, including every feature under a capability tagged "(will be created)"). The requirement narrative is NOT a restatement of the feature name or a generic one-liner - it must capture the actual detail the PM described in chat (specific behaviors, edge cases, operational definitions), one to a few sentences, retained and refined turn over turn as the conversation adds detail. This narrative is the PRIMARY source downstream feature-generation and story-generation will ground in once Finalize creates the capability shell - if this section only lists names, that detail is lost permanently.\n'
    + '- "## 6. Target Users": one "- <Persona Name>: <one-line description>" bullet per user segment this release is for (typically 2-4) - who they are and how they relate to the capabilities/journeys below. Mark inferred bullets "(inferred - confirm with PM)".\n'
    + '- "## 7. User Journeys": one "### <Journey Name>" sub-heading per key end-to-end scenario, each tied to a persona from "## 6. Target Users" and this release\'s "## 4. Capabilities", followed by numbered steps ("1. ", "2. ", ...) walking through the experience from the user\'s perspective. Mark inferred journeys/steps "(inferred - confirm with PM)".\n'
    + '- "## 8. Non-Functional Requirements": bullets in the exact format "- **<Category>:** <detail>", drawing only from categories genuinely relevant to this release (e.g. Performance, Security, Scalability, Availability, Privacy, Accessibility) - never pad with irrelevant categories, but never leave the section fully empty either (state specifically what is still needed in one sentence if truly nothing yet).\n'
    + '- "## 9. Out of Scope": what is explicitly excluded from this release (bullet list - be specific, not "everything else").\n'
    + '- "## 10. Assumptions": two sub-lists. "Assumptions made so far" - every assumption the agent has made to fill a gap the user has not addressed, each bullet STARTING WITH THE LITERAL PREFIX "**Assumed:** " followed by the assumption in plain language, specific to this release (e.g. "**Assumed:** Points are credited only after order confirmation.") - do not repeat the literal openQuestions text here as an assumption, and never write an assumption bullet without the "**Assumed:**" prefix so a PM can visually distinguish agent-guessed content from explicit PM intent at a glance. "Risks and dependencies" - known risks, dependencies, or constraints worth flagging to engineering/design before work starts, each bullet STARTING WITH THE LITERAL PREFIX "**Risk:** ".\n'
    + '- "## 11. Open Questions": a bullet mirroring each entry in the openQuestions field below, so the brief itself shows the same unresolved items visible in the openQuestions array (never more, never fewer).\n'
    + 'Apply the "(inferred - confirm with PM)" tag to any bullet/statement in ANY of the 11 sections above that the PM has not yet explicitly confirmed - not only Success Criteria.\n'
...
function buildRequirementAgentTurnPrompt(sessionContext,liveDraftMd,chatHistory,userMessage,docContext,uploadedDocText,uploadedDocName){
  ...
  const sys='You are continuing a Requirement Agent conversation, refining a release-scoped Live Draft that already has real content. '
    ...
    + 'When the requirements you are discussing do not map to any EXISTING capability listed below, say so in plain conversational text (never a structured card, button, or special message type) and tag that capability "will be created — under: <Metric or Process Area Name>" (this exact phrase, never bare "new") in the Live Draft\'s "## 4. Capabilities" section, under its own sub-heading - naming a real EXISTING Discovery Map metric/process area (copied verbatim from the list below) when one genuinely fits, or a specific new proposed name (never a generic placeholder like "Custom Metric") only when none does. '
  ...
  const usr='DISCOVERY MAP (every value chain stage, with its metrics/process areas — the only valid source of an EXISTING metric/process area name for the "will be created — under:" tag):\n'+dmStr+'\n\n'
    ...
    + (uploadedDocText
      ...
      : ('THE USER JUST SAID:\n'+userMessage+'\n\n'))
    + 'TASK: Reply conversationally (chatReply) - never invent your own greeting here, this is a continuing turn. Update the Live Draft (liveDraftMd), keeping ALL of its sections current and in the same order: "## 1. Requirement Summary", "## 2. Problem Statement", "## 3. Success Criteria", "## 4. Capabilities" (one sub-section per touched capability, each tagged either "(existing)" or "(will be created — under: <Metric or Process Area Name>)" exactly), "## 5. Features", "## 6. Target Users", "## 7. User Journeys", "## 8. Non-Functional Requirements", "## 9. Out of Scope", "## 10. Assumptions", "## 11. Open Questions". See the section-content rules below for what belongs in each - do not drop a section just because this turn is not about it.\n\n'
    + _raSectionContentRules()+'\n\n'
    + 'CRITICAL - openQuestions consistency: the openQuestions array below must be the EXACT set of clarifying questions you are still waiting on the user to answer, no more and no fewer. Never include a question the user\'s latest message already answered. Never include a question that is not also reflected in the Live Draft\'s "## 11. Open Questions" section. If your chatReply text numbers or lists specific open questions to the user, the openQuestions array must contain exactly those same questions, same count, same order - a mismatch between what you show the user and what you return in this field is treated as a bug.\n\n'
    + _raClarifyingQuestionsRules()+'\n\n'
    + 'Return ONLY valid JSON with these exact fields:\n'
    + '{\n'
    + '  "chatReply": "conversational message, plain text with \\n for line breaks, no markdown headings",\n'
