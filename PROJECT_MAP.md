...
| Voice dictation — Requirement Agent's attachment (settings flag, tab-leave/re-render/session-clear cleanup) | `scripts/requirement-agent.js` (raRenderCenter, raResetState), `scripts/api.js` (switchTab prev==='ra' hook), `scripts/left-panel.js` (applyFeats kill switch) |
