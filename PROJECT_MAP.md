...
- **Voice dictation (v9.24):** `_raVoiceSupported()`/`_raVoiceInit()`/`raVoiceToggle()`/`_raVoiceStop()`/`_raVoiceRenderButtonState()`/`_raVoiceOnStart()`/`_raVoiceOnResult()`/`_raVoiceOnError()`/`_raVoiceOnEnd()` — browser-native `SpeechRecognition`, gated behind `appSettings.featVoiceInput` (default `false`, see `state.js`) AND feature detection. `_raVoiceUiState` (`'connecting'` while `.start()` has been called but the browser hasn't confirmed audio capture yet — e.g. behind a first-use mic-permission prompt — vs `'listening'` once `onstart` actually fires) drives a small status label + a distinct button look, separate from the simpler `raVoiceListening` boolean every cleanup hook checks; deliberately NOT reset on Chrome's silent onend-triggered auto-restart, to avoid flickering the label on every silence-timeout cycle. dictation streams into the existing `#ra-chat-input`, Send still gates the commit through the unchanged `_raRunTurn()` pipeline, no new AI/prompt work. All state (`raVoiceListening`, the live recognition instance, the committed-transcript buffer) is view-scoped only, declared in this file, never on `conv{}` and never persisted. Five cleanup call sites, all using `_raVoiceStop('abort')` except the plain user-toggle-off (`'stop'`): the top of `raRenderCenter()` (the sole function, 3 call sites all in this file, that rebuilds `#ra-chat-input`'s DOM node), `raResetState()` (session clear — the *only* cleanup point reached during the live-sync kickout path, since `home.js`'s `homeClearSession()` wipes `#ra-tab`'s DOM before calling it), a `prev==='ra'` tab-leave hook in `api.js`'s `switchTab()` (no such hook existed before this — covers in-app tab-row navigation only), `left-panel.js`'s `applyFeats()` (settings-flag-off kill switch — required because `openSettingsPage()`/`closeSettingsPage()` never call `switchTab()`, so no other hook would catch the flag flipping off while Settings is open on top of an active session), and a page-lifetime `document.addEventListener('visibilitychange', ...)` registered directly in this file (confirmed via live testing — none of the above cover the BROWSER tab/window itself losing visibility while the page keeps running in the background; no auto-resume on becoming visible again, by design — first `visibilitychange`/`blur` listener anywhere in this codebase, confirmed via grep, so no existing pattern was available to reuse).
...
| Requirement Agent voice dictation (v9.24) — toggle, settings flag, cleanup lifecycle | `scripts/requirement-agent.js` (raVoiceToggle, _raVoiceStop and related), `scripts/state.js` (appSettings.featVoiceInput), `scripts/settings-page.js` (spP1, settingsPageSave), `scripts/api.js` (switchTab prev==='ra' hook), `scripts/left-panel.js` (applyFeats kill switch) |
