...
- **Voice dictation (v9.25):** `#regen-in`'s mic button rendered via `voiceButtonHtml({textareaId:'regen-in',...})` (`scripts/voice-input.js`) inside `renderDiagnosticActionBar()`'s `.diag-refine-row`. `renderDiagnosticActionBar()` itself calls `voiceStopActive('abort')` at its top — confirmed via grep this function does `existing.remove()`+full rebuild on far more than just tree regeneration (also `confirmDeleteStage()` and the equivalent add/edit/delete-stage/capability paths), so any tree mutation anywhere on this screen would otherwise silently kill an active dictation session without this guard. `regen()` (the refine bar's send-icon handler) and `toggleRefineBar()`'s collapse path both call `voiceStopActive('abort')` too — stop-on-send and stop-on-collapse, a deliberate divergence from Requirement Agent's persistent-across-sends behavior, since this is a single-shot refine box, not a multi-turn chat.
...
| Voice dictation — shared module (any surface), toggle/stop primitives, cleanup contract | `scripts/voice-input.js` (voiceButtonHtml, voiceToggle, voiceStopActive), `scripts/state.js` (appSettings.featVoiceInput), `scripts/settings-page.js` (spP1, settingsPageSave) |
