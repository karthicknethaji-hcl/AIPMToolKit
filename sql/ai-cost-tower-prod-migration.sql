-- ═══════════════════════════════════════════════════════════════════
-- AI Cost Control Tower: consolidated PROD migration
-- Run in Supabase SQL editor, PROD ONLY, in one pass. NOT run by Claude
-- Code — Nethaji runs this against Supabase on his own timeline.
--
-- This consolidates three files already applied to pgt-dev incrementally,
-- in this order:
--   1. sql/ai-cost-tower.sql                  (core v1 data model)
--   2. sql/ai-cost-tower-build-b-addendum.sql (adds cache_read_price_per_mtok)
--   3. sql/ai-cost-tower-cache-cost-fix.sql   (fixes calculated_cost
--      double-counting cache-read tokens for OpenAI/Gemini)
-- Those three files remain on disk as the historical record of what
-- actually happened on dev, step by step — do not delete them. This file
-- exists only because prod has never run any of the three yet, so there's
-- no reason to replay dev's intermediate history (including a DROP+CREATE
-- that only existed because dev already had an older function signature
-- to work around). Prod goes straight to the final, correct shape below.
--
...
-- v1.1 manual governance enforcement (sql/ai-cost-tower-governance.sql on
-- dev): anchor timestamp for auto-reverting a Restrict/Stop selection back
-- to Notify Only at the start of the next monthly period. Folded in here,
-- not left as a fourth file, since this consolidated migration has not
-- been run against prod yet and is meant to go straight to final shape.
...
-- Populate tier per model. Values below are read directly from this app's
-- own single source of truth, TIER_MODEL_BY_PROVIDER (scripts/api.js:330-351),
-- not guessed or copied from dev's own UPDATE statements (never seen by
-- Claude Code — dev's were written and run directly). Gemini has no
-- premium-tier model (TIER_MODEL_BY_PROVIDER's own comment: "premium: null
-- is intentional, not a placeholder"), so there is no Gemini frontier row
-- below, on purpose. Each UPDATE is a no-op (0 rows) if prod's
-- mt_model_pricing has no row for that (provider, model_name) pair yet —
-- safe, but check the post-flight query below rather than assuming all 8
-- landed.
UPDATE mt_model_pricing SET tier = 'economical', tier_sort_order = 1 WHERE provider = 'anthropic' AND model_name = 'claude-haiku-4-5';
UPDATE mt_model_pricing SET tier = 'balanced',   tier_sort_order = 2 WHERE provider = 'anthropic' AND model_name = 'claude-sonnet-4-6';
UPDATE mt_model_pricing SET tier = 'frontier',   tier_sort_order = 3 WHERE provider = 'anthropic' AND model_name = 'claude-opus-4-8';
UPDATE mt_model_pricing SET tier = 'economical', tier_sort_order = 1 WHERE provider = 'openai'    AND model_name = 'gpt-5.6-luna';
UPDATE mt_model_pricing SET tier = 'balanced',   tier_sort_order = 2 WHERE provider = 'openai'    AND model_name = 'gpt-5.6-terra';
UPDATE mt_model_pricing SET tier = 'frontier',   tier_sort_order = 3 WHERE provider = 'openai'    AND model_name = 'gpt-5.6-sol';
UPDATE mt_model_pricing SET tier = 'economical', tier_sort_order = 1 WHERE provider = 'gemini'    AND model_name = 'gemini-3.5-flash-lite';
UPDATE mt_model_pricing SET tier = 'balanced',   tier_sort_order = 2 WHERE provider = 'gemini'    AND model_name = 'gemini-3.6-flash';

-- ─── Pre-flight: confirm current access on the two pre-existing tables ─
-- mt_ai_usage_events and mt_model_pricing are written only by the
-- service-role client today and have no confirmed read grant for
-- `authenticated`. Run this BEFORE the function below and read the
-- result, don't assume the answer (dev's answer isn't necessarily prod's):
--
...
-- Final shape (dev arrived here via three incremental steps; prod creates
-- it directly): 26 return columns including cache_read_price_per_mtok
-- (Build B addendum) and a calculated_cost formula that correctly handles
-- Anthropic vs. OpenAI/Gemini's different cache-token accounting (cache
-- cost fix) — Anthropic's input_tokens excludes cache reads (bill in
-- full), OpenAI/Gemini's already includes them (bill only the non-cached
-- remainder at full price, to avoid double-billing the cached portion).
-- Every grouping and aggregate in the spec (Sections 4-6) is computed
-- client-side in scripts/cost-tower.js from this function's result, not
-- in SQL.
...
-- Consolidated from all three source files' own validation sections.
-- ═══════════════════════════════════════════════════════════════════
...
-- 2. tier / tier_sort_order columns exist on mt_model_pricing, AND confirm
--    how many of the 8 backfill UPDATEs above actually matched a row —
--    don't assume all 8 landed just because none of them errored:
-- SELECT column_name, data_type FROM information_schema.columns
-- WHERE table_name = 'mt_model_pricing' AND column_name IN ('tier','tier_sort_order');
-- SELECT provider, model_name, tier, tier_sort_order FROM mt_model_pricing ORDER BY provider, tier_sort_order;
-- -- any (provider, model_name) pair from the 8 UPDATEs above that's missing
-- -- here means prod's mt_model_pricing has no row for that model yet —
-- -- not an error, just something to know before relying on tier displays.
...
--    one known row, including a cache-read row for a non-Anthropic provider
--    once such data exists), and raises the authorization exception for a
--    non-admin:
-- SELECT provider, input_tokens, cache_read_tokens, cache_read_price_per_mtok, calculated_cost
-- FROM mt_ai_cost_events_list('<company>', '<period_start>', '<period_end>') LIMIT 5;
